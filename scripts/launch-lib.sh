# Shared by MedScroll.app and start.command: first-run setup, port resolution, starting.
#
...
setup_backend() {
  local stamp="$REPO/backend/.venv/.medscroll-synced" lock="$REPO/backend/uv.lock"
...
setup_frontend() {
  ...
  stamp="$fe/node_modules/.medscroll-installed"; lock="$fe/package-lock.json"
