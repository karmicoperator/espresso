...
export function LoadingState({
  ...
  return (
    ...
    >
      <div
        className={`${spinner} animate-spin rounded-full border-4 border-white/20 border-t-blue-400`}
      />
      <p className={`mt-4 ${text} text-white/60`}>{message}</p>
    </div>
  );
}
...
export function LoadingSpinner({
  ...
  return (
    <div
      className={`${spinner} animate-spin rounded-full border-4 border-white/20 border-t-blue-400 ${className}`}
    />
  );
}
...
export function CardSkeleton({ className = "" }: { className?: string }) {
  return (
    <div
      className={`rounded-2xl bg-white/5 p-6 ring-1 ring-white/10 ${className}`}
    >
